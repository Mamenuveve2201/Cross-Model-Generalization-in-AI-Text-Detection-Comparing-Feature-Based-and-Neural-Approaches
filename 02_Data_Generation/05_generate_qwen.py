...
print(f"\n🚀 Generating 1,000 Qwen essays with MATCHED topics...")
...
result_df.to_csv('data/raw/qwen_samples.csv', index=False)

print(f"\n✅ COMPLETE!")
successful = len(result_df[~result_df['generated_text'].str.contains('FAILED', na=False)])
print(f"Success rate: {successful/len(result_df)*100:.1f}%")S